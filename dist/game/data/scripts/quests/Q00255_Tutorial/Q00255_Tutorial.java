	...
	public String onEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.startsWith("QT"))
		{
			if (Ex == -2)
			{
				...
				// Bizim quest'i başlat
				try
				{
					Q00801_MonsterHunterSpecialization.newCharacterCreated(player);
					System.out.println("Attempting to start Monster Hunter quest for: " + player.getName());
				}
				catch (Exception e)
				{
					System.out.println("Error starting Monster Hunter quest: " + e.getMessage());
					e.printStackTrace();
				}
			}
			else if (Ex == -3)
