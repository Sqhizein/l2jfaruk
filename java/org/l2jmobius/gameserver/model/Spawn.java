...
import org.l2jmobius.gameserver.ai.BossChampionAI;
...
public class Spawn extends Location implements IIdentifiable, INamable
{
	...
	private Npc initializeNpc(Npc npc)
	{
		...
		// Check and set champion status for monster
		if (Config.CHAMPION_ENABLE && npc.isMonster() && !npc.isQuestMonster() && !_template.isUndying() && !npc.isRaid() && !npc.isRaidMinion())
		{
			int npcLevel = npc.getLevel();
			
			if ((Config.BOSS_CHAMPION_ENABLE) && (npcLevel >= Config.BOSS_CHAMPION_MIN_LEVEL) && (npcLevel <= Config.BOSS_CHAMPION_MAX_LEVEL) && (Config.CHAMPION_ENABLE_IN_INSTANCES || (getInstanceId() == 0)) && (Rnd.get(1000) < (Config.BOSS_CHAMPION_FREQUENCY * 10)))
			{
				((Attackable) npc).setChampion(true);
				((Attackable) npc).setChampionType(6);
				
				// Boss Champion Skills
				if (Config.BOSS_CHAMPION_USE_SKILLS)
				{
					try
					{
						Monster bossChamp = (Monster) npc;
						
						// Özel AI'yi ayarla
						bossChamp.setAI(new BossChampionAI(bossChamp));
						
						// Skill'leri ekle
						for (SkillHolder sh : Config.BOSS_CHAMPION_SKILLS)
						{
							try
							{
								...
								if (skill != null)
								{
									bossChamp.addSkill(skill);
									bossChamp.enableSkill(skill);
									LOGGER.info("Added skill ID: " + skill.getId() + " Level: " + skill.getLevel() + " to Boss Champion: " + npc.getId());
								}
							}
							catch (Exception e)
							{
								LOGGER.warning("Error adding skill " + sh.getSkillId() + " to Boss Champion: " + e.getMessage());
							}
						}
						
						LOGGER.info("Total skills added: " + bossChamp.getSkills().size() + " to Boss Champion: " + npc.getId());
					}
					catch (Exception e)
					{
						LOGGER.warning("Error setting up Boss Champion skills: " + e.getMessage());
					}
				}
				
				// Follower spawn kısmı
				try
				{
					Location spawnLoc = npc.getLocation();
					if (spawnLoc != null)
					{
						for (int champType = 1; champType <= 5; champType++)
						{
							int radius = Config.BOSS_CHAMPION_FOLLOWER_RANGE;
							int attempts = 10;
							
							while (attempts > 0)
							{
								int x = spawnLoc.getX() + (Rnd.get(-radius, radius));
								int y = spawnLoc.getY() + (Rnd.get(-radius, radius));
								int z = GeoEngine.getInstance().getHeight(x, y, spawnLoc.getZ());
								
								if (GeoEngine.getInstance().canSeeTarget(spawnLoc.getX(), spawnLoc.getY(), spawnLoc.getZ(), x, y, z))
								{
									try
									{
										Spawn spawn = new Spawn(npc.getId());
										spawn.setXYZ(x, y, z);
										
										Monster follower = (Monster) spawn.doSpawn();
										if (follower != null)
										{
											follower.setChampion(true);
											follower.setChampionType(champType);
											LOGGER.info("Spawned Boss Champion follower Type: " + champType + " at X:" + x + " Y:" + y + " Z:" + z);
											break;
										}
									}
									catch (Exception e)
									{
										LOGGER.warning("Error spawning Boss Champion follower: " + e.getMessage());
									}
								}
								attempts--;
							}
				...
				{
					LOGGER.warning("Error in Boss Champion spawn process: " + e.getMessage());
				}
			}
			// Easy Champion
			if ((Config.EASY_CHAMPION_FREQUENCY > 0) && (npcLevel >= Config.EASY_CHAMPION_MIN_LEVEL) && (npcLevel <= Config.EASY_CHAMPION_MAX_LEVEL) && (Config.CHAMPION_ENABLE_IN_INSTANCES || (getInstanceId() == 0)) && (Rnd.get(1000) < (Config.EASY_CHAMPION_FREQUENCY * 10)))
			{
				((Attackable) npc).setChampionType(1); // Önce type'ı set et
				((Attackable) npc).setChampion(true); // Sonra champion yap
			}
			// Normal Champion
			else if ((Config.CHAMPION_FREQUENCY > 0) && (npcLevel >= Config.CHAMP_MIN_LEVEL) && (npcLevel <= Config.CHAMP_MAX_LEVEL) && (Config.CHAMPION_ENABLE_IN_INSTANCES || (getInstanceId() == 0)) && (Rnd.get(1000) < (Config.CHAMPION_FREQUENCY * 10)))
			{
				((Attackable) npc).setChampionType(2);
				((Attackable) npc).setChampion(true);
			}
			// Hard Champion
			else if ((Config.HARD_CHAMPION_FREQUENCY > 0) && (npcLevel >= Config.HARD_CHAMPION_MIN_LEVEL) && (npcLevel <= Config.HARD_CHAMPION_MAX_LEVEL) && (Config.CHAMPION_ENABLE_IN_INSTANCES || (getInstanceId() == 0)) && (Rnd.get(1000) < (Config.HARD_CHAMPION_FREQUENCY * 10)))
			{
				((Attackable) npc).setChampionType(3);
				((Attackable) npc).setChampion(true);
			}
			// Very Hard Champion
			else if ((Config.VERY_HARD_CHAMPION_FREQUENCY > 0) && (npcLevel >= Config.VERY_HARD_CHAMPION_MIN_LEVEL) && (npcLevel <= Config.VERY_HARD_CHAMPION_MAX_LEVEL) && (Config.CHAMPION_ENABLE_IN_INSTANCES || (getInstanceId() == 0)) && (Rnd.get(1000) < (Config.VERY_HARD_CHAMPION_FREQUENCY * 10)))
			{
				((Attackable) npc).setChampionType(4);
				((Attackable) npc).setChampion(true);
			}
			// Catastrophic Champion
			else if ((Config.CATASTROPHIC_CHAMPION_FREQUENCY > 0) && (npcLevel >= Config.CATASTROPHIC_CHAMPION_MIN_LEVEL) && (npcLevel <= Config.CATASTROPHIC_CHAMPION_MAX_LEVEL) && (Config.CHAMPION_ENABLE_IN_INSTANCES || (getInstanceId() == 0)) && (Rnd.get(1000) < (Config.CATASTROPHIC_CHAMPION_FREQUENCY * 10)))
			{
				((Attackable) npc).setChampionType(5);
				((Attackable) npc).setChampion(true);
			}
			// Boss Champion
			else if ((Config.BOSS_CHAMPION_ENABLE) && (npcLevel >= Config.BOSS_CHAMPION_MIN_LEVEL) && (npcLevel <= Config.BOSS_CHAMPION_MAX_LEVEL) && (Config.CHAMPION_ENABLE_IN_INSTANCES || (getInstanceId() == 0)) && (Rnd.get(1000) < (Config.BOSS_CHAMPION_FREQUENCY * 10)))
			{
				((Attackable) npc).setChampionType(6);
				((Attackable) npc).setChampion(true);
			}
	...
	public void respawnNpc(Npc oldNpc)
	{
		if (_doRespawn)
		{
			// Eğer yaratık Attackable ise champion özelliklerini sıfırla
			if (oldNpc instanceof Attackable)
			{
				((Attackable) oldNpc).setChampion(false);
				((Attackable) oldNpc).setChampionType(0);
			}
			// Yeni spawn gibi baştan başlat
