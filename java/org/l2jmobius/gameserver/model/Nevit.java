...
public class Nevit implements IUniqueId
{
	...
	public void startAdventTask()
	{
		if (_adventTask == null)
		{
			synchronized (this)
			{
				...
				{
					_adventTask = ThreadPool.schedule(new AdventTask(), 1000);
	...
	public synchronized void startNevitEffect(int timeValue)
	{
		...
		if ((Config.NEVIT_IGNORE_ADVENT_TIME || (getAdventTime() < Config.NEVIT_ADVENT_TIME)) && (time > 0))
		{
			...
			_player.startAbnormalVisualEffect(false, AbnormalVisualEffect.NAVIT_ADVENT);
	...
	public class NevitEffectEnd implements Runnable
	{
		...
		public void run()
		{
			...
			_player.stopAbnormalVisualEffect(false, AbnormalVisualEffect.NAVIT_ADVENT);
			stopNevitEffectTask(false);
		}
	}
