...
public class SellBuffsManager implements IXmlReader
{
	...
	private String getEnchantType(Skill skill) {
		int level = skill.getLevel();
		if (level >= 101 && level <= 130) {
			return "Time";
		} else if (level >= 201 && level <= 230) {
			return "Cost";
		} else if (level >= 301 && level <= 330) {
			return "Power";
		} else {
			return "none"; // Bu varsayılan olarak belirtilmeyen seviyelerde "none" dönecek
		}
	}
	...
	private String buildBuffMenu(Player seller, int index)
	{
		...
		for (SellBuffHolder holder : sellList)
		{
			...
			sb.append("<td align=left>" + skill.getName() + (skill.getLevel() > 100 ? "<font color=\"LEVEL\"> + " + (skill.getLevel() % 100) + " " + getEnchantType(skill) + "</font></td>" : "</td>"));
