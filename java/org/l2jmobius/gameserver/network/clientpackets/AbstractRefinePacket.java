...
public abstract class AbstractRefinePacket extends ClientPacket
{
	...
	static
	{
		...
		_lifeStones.put(52008, new LifeStone(GRADE_ACC, 0));   // Life Stone Acc 20 Lv
		_lifeStones.put(52004, new LifeStone(GRADE_NONE, 0));  // Life Stone 20 Lv
        _lifeStones.put(52005, new LifeStone(GRADE_MID, 0));   // Life Stone Mid 20 Lv
        _lifeStones.put(52006, new LifeStone(GRADE_HIGH, 0));  // Life Stone High 20 Lv
        _lifeStones.put(52007, new LifeStone(GRADE_TOP, 0));   // Life Stone Top 20 Lv
	...
	protected static boolean isValid(Player player, Item item)
	{
		...
		if (item.getTemplate().getCrystalType().isLesser(CrystalType.D))
		{
			return false;
		}
