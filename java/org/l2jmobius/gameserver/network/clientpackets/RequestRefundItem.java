...
public class RequestRefundItem extends ClientPacket
{
	...
	protected void runImpl()
	{
		...
		for (int i = 0; i < _items.length; i++)
		{
			...
			// Yeni fiyat hesaplama kodu - Aynı yuvarlama mantığı ile
			long basePrice = template.getReferencePrice() / 2; // Önce bölme işlemi (15.5 -> 15)
			long price = basePrice; // Yuvarlanmış hali (15)
			if (!Config.MERCHANT_ZERO_SELL_PRICE && Config.PREMIUM_SYSTEM_ENABLED && player.hasPremiumStatus())
			{
				price = (long) (basePrice * Config.PREMIUM_RATE_SELL_PRICE); // 15 * 1.25 = 18.75 -> 18
			}
			adena += count * price;
