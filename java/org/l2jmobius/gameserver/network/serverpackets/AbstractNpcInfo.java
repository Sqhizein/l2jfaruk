...
public abstract class AbstractNpcInfo extends ServerPacket
{
	...
	public static class NpcInfo extends AbstractNpcInfo
	{
		...
		public void writeImpl(GameClient client, WritableBuffer buffer)
		{
			...
			// Bu kısmı tamamen aşağıdaki kod ile değiştirin:
			if (_npc.isMonster())
			{
				StringBuilder titleBuilder = new StringBuilder();
				
				// Önce Champion kontrolü yap
				if (_npc.isChampion())
				{
					switch (((Attackable) _npc).getChampionType())
					{
						case 1:
							titleBuilder.append(Config.EASY_CHAMPION_TITLE);
							break;
						case 2:
							titleBuilder.append(Config.CHAMP_TITLE);
							break;
						case 3:
							titleBuilder.append(Config.HARD_CHAMPION_TITLE);
							break;
						case 4:
							titleBuilder.append(Config.VERY_HARD_CHAMPION_TITLE);
							break;
						case 5:
							titleBuilder.append(Config.CATASTROPHIC_CHAMPION_TITLE);
							break;
						case 6:
							titleBuilder.append(Config.BOSS_CHAMPION_TITLE);
							break;
					}
				}
				...
				if (Config.SHOW_NPC_LEVEL)
				{
					...
					titleBuilder.append("Lv ").append(_npc.getLevel());
				}
				
				// Aggression bilgisini ekle
				if (Config.SHOW_NPC_AGGRESSION)
				{
					...
					{
						titleBuilder.append("[G]");
					}
				}
				
				// Orijinal title'ı ekle
				if ((_npc.getTitle() != null) && !_npc.getTitle().isEmpty())
				{
					if (titleBuilder.length() > 0)
					{
						titleBuilder.append(" ");
					}
					titleBuilder.append(_npc.getTitle());
				}
